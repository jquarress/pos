...
<div class="right" style="overflow-y:auto">
  ...
  <form  method="post" action="#" > 
	      <input  type="text" name="searchinput2" placeholder="Enter State Id here"> 
...
<table class="table table-striped" style="overflow: scroll">
    ...
    <div style ="overflow: scroll">
        ...
        if(mysqli_connect_error()){
            ...
            $sql = "SELECT stateid, fullname, phone 
            from customers Where stateid LIKE '%$searchinput%' or 
            fullname LIKE '%$searchinput%'";
            $result = $conn-> query($sql);

            if($result-> num_rows > 0){
                while($row = $result-> fetch_assoc()){
                    echo "<tr><td>". $row["stateid"] . "</td>
                    <td>". $row["fullname"] . "</td>
                    <td>". $row["phone"] . 
                   '<td><input type="submit" name="checkinbutton'.'" value="check in"/> </td></tr>';
                }
                echo "</table>";
            }else{
                echo "no patient was found from this search";
            }
            $conn->close();
         }
        }
